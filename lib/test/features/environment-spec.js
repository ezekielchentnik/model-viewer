 ...
 * @param {Object} meta
 */
const waitForEnvmap = (model, meta) => waitForEvent(model, 'envmap-change', e => textureMatchesMeta(e.value, Object.assign({}, meta, { type: 'EnvironmentMap' })));
/**
 ...
 * @see textureMatchesMeta
 */
const waitForLoadAndEnvmap = (scene, element, meta) => Promise.all([waitForEvent(element, 'load'), waitForEnvmap(scene.model, meta)]);
suite('ModelViewerElementBase with EnvironmentMixin', () => {
    ...
    test('has default skysphere if no background-image or background-color', () => {
        expect(skysphereUsingColor(scene, 'ffffff')).to.be.equal(true);
    });
    test('has default skysphere if no background-image or background-color when in DOM', async () => {
    ...
    suite('with a background-color property', () => {
        suite('and a src property', () => {
            ...
            test('displays skysphere with correct color after attaching to DOM', async function () {
                document.body.appendChild(element);
                await timePasses();
                expect(skysphereUsingColor(scene, 'ff0077')).to.be.ok;
            });
