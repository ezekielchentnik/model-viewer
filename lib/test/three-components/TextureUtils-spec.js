...
suite('TextureUtils', () => {
    ...
    suite('load', () => {
        test('loads a valid texture from URL', async () => {
            ...
            expect(textureMatchesMeta(texture, { type: 'Equirectangular', url: EQUI_URL }))
                .to.be.ok;
        });
    ...
    suite('equirectangularToCubemap', () => {
        test('creates a cubemap from texture', async () => {
            ...
            expect(textureMatchesMeta(cubemap, { type: 'EnvironmentMap', url: EQUI_URL }))
                .to.be.ok;
        });
    ...
    suite('toCubemapAndEquirect', () => {
        test('returns a cubemap and texture from url', async () => {
            ...
            expect(textureMatchesMeta(textures.equirect, { type: 'Equirectangular', url: EQUI_URL }))
                .to.be.ok;
            expect(textureMatchesMeta(textures.cubemap, { type: 'EnvironmentMap', url: EQUI_URL }))
                .to.be.ok;
        });
