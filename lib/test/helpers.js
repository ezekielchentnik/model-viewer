...
export const elementFromLocalPoint = (document, x, y) => {
    const host = (document === window.document) ? window.document.body : document.host;
 ...
 * @return {boolean}
 */
export const textureMatchesMeta = (texture, meta) => !!(texture && texture.userData && Object.keys(meta).reduce((matches, key) => {
 ...
 */
export const waitForEvent = (target, eventName, predicate) => new Promise(resolve => {
    function handler(e) {
